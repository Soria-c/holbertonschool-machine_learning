...
class Decision_Tree():
    ...
    def Gini_split_criterion_one_feature(self, node, feature):
            ind_c_node = self.explanatory[:, feature][node.sub_population]
            p_thresh = self.possible_thresholds(node, feature)
            classes = self.target[node.sub_population]
            matrix = np.zeros((ind_c_node.size, p_thresh.size, classes.size), dtype=bool)
